...
def get_parser():
    ...
    parser.add_argument(
        ...
        default=os.path.join(here, "instances-google.csv"),
    )
    ...
    gen.add_argument("--region", help="set region for cloud", default="us-central1")
...
def select_instances(
    ...
    # Read in data frame
    subset = pandas.read_csv(datafile, index_col=0)
...
def generate_data(args):
    ...
    cli = Client(use_cache=not args.no_cache, clouds=["google"])
    ...
    for cloud in cli.get_clouds():
        ...
        # Note that this returns raw data
        if args.no_cache:
            instances = cloud.instances()
            prices = cloud.prices()

        else:
            instances = cli.instances()["google"]
            prices = cli.prices()["google"]

        # Add prices to instances (for those we can)
        instances.add_instance_prices(prices)

        # There is more data here, this is what we want for now
        df = instances_to_table(instances)
        print(df)
        print(f"Saving {df.shape[0]} instances in table to {args.data}")
        df.to_csv(args.data)


def instances_to_table(instances):
    """
    ...
    # Put into pandas data frame so we can sort / filter by things
    idx = 0
    for i in instances.data:
        ...
        vcpus = i['guestCpus']
        cpus = vcpus / 2
        gpu = "accelerators" in i
        memory_mb = i['memoryMb']

        # Skip those we don't have price / spot for, for now
        price = i.get('price')
        spot_price = i.get('spot_price')
        if not price or not spot_price:
            continue
